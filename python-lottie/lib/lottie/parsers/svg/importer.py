...
def parse_color(color, current_color=Color(0, 0, 0, 1)):
    ...
    """
    # sunglasses fix
    if color.startswith("url(#linearGradient-"):
        color = "#252C2F"
        return Color(int(color[1:3], 16) / 0xff, int(color[3:5], 16) / 0xff, int(color[5:7], 16) / 0xff, 0.5)
...
class SvgParser(SvgHandler):
    def __init__(self, name_mode=NameMode.Inkscape):
        ...
        self.skinColor = "#EDB98A"
        self.clothesColor = "#3C4F5C"
        self.hairColor = "#ECDCBF"
        self.beardColor = "#ECDCBF"
    ...
    def parse_etree(self, etree, layer_frames=0, is_file=False, *args, **kwargs):
        ...
        #   Возможно проблема где-то в SVG, в границах фигур которые красим, можем не красить вообще ???
    ...
    def _parse_unit(self, value):
        ...
        return float(value.rstrip('%')) * mult
    ...
    def parse_style(self, element, parent_style):
        ...
        colors = [
          "#614335", # skinColor BLACK
          "#A55728", # pyavata with text
          "#FF488E", # PINK pyavata with text tshirt
          "#F59797", # PASTEL PINK hair color
          "#E8E1E1", # SILVER_GRAY beard          
        ]

        if 'fill' in style.keys():
          if style["fill"] in colors:
            style["fill"] = 'transparent'
          elif style["fill"] == "#D0C6AC": # skin after change
             style["fill"] = self.skinColor 
          elif style["fill"] == "#E6E6E6" or style["fill"] == "#9368ff": # tshirt after change + custom svg's
            style["fill"] = self.clothesColor 
          elif style["fill"] == "#944F23" or style["fill"] == "#28354B" or style["fill"] == "#1F3140": # hair after change
            style["fill"] = self.hairColor 
          elif style["fill"] == "#5E7AA9": # beard color in svg
            style["fill"] = self.beardColor 
          elif style["fill"] == "#FFF": 
            style["fill"] = "#FFFFFF"
         #  else:
         #    print(style["fill"])
...
def parse_svg_etree(etree, layer_frames=0, is_file=False, colors={
  "skinColor": "#EDB98A",
  "clothesColor": "#3C4F5C",
  "hairColor": "#FF488E",
  "beardColor": "#FF488E"
    ...
    parser = SvgParser()
    parser.skinColor = colors["skinColor"]
    parser.clothesColor = colors["clothesColor"] 
    parser.hairColor = colors["hairColor"] 
    parser.beardColor = colors["beardColor"] 
    return parser.parse_etree(etree, layer_frames, is_file, *args, **kwargs)


def parse_svg_file(file, layer_frames=0, *args, **kwargs):
    return parse_svg_etree(ElementTree.parse(file), layer_frames, is_file=True, *args, **kwargs)
